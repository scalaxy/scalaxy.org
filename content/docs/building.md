...
The suite uses plain assertions rather than a test framework, so it runs anywhere SBCL runs, including CI. It exercises real sockets (TCP and HTTP on ephemeral ports), crash recovery through log replay, and the openCypher engine end to end (TCP, gateway, and web layers included).
